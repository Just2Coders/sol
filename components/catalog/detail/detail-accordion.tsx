 ...
 *
 * El pliegue entre filas va en `border-foreground` —la misma tinta del texto,
 * no un gris de utilería— porque es la línea que más pesa de la columna: la
 * que corta una sección de la siguiente.
 */
 ...
 *    (`border-foreground`). Las de dentro del contenido siguen en
 ...
 * 3. **El rótulo abierto se enciende** a `text-foreground` y deja de ser
...
export function CatalogDetailAccordionRow({
  ...
  return (
    <AccordionPrimitive.Item value={value} className="border-foreground border-b">
      <AccordionPrimitive.Header className="flex">
        <AccordionPrimitive.Trigger className="group text-muted-foreground data-[state=open]:text-foreground data-[state=closed]:hover:bg-background data-[state=closed]:hover:text-foreground text-heading-3 lg:px-gutter focus-visible:outline-ring ease-standard flex flex-1 items-center gap-2.5 px-6 py-5 transition-colors duration-base focus-visible:outline-2 focus-visible:-outline-offset-2">
          <span className="flex-1 text-left">{label}</span>
          <ChevronDown
            aria-hidden
            className="ease-standard size-4 shrink-0 transition-transform duration-base group-data-[state=open]:rotate-180 motion-reduce:transition-none"
          />
      ...
      <AccordionPrimitive.Content className="data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down ease-standard overflow-hidden duration-base">
        <div className="lg:px-gutter px-6 pb-8">{children}</div>
